...
population <- read_csv('/home/baron/Documents/rotation_2/QM_rotation/scripts/outputs/vcfs/all_2.csv',
...
cage_plot <- ggplot(population, aes(x = Generation, y = Minor_allele_frequency, color = Cage)) + geom_line() +
  theme_grey() +
  labs(title = 'Alternate allele frequency per generation \nNeutrality (h=0.5, s=0)', y = 'Alternate allele frequency', x = 'Generations') +
